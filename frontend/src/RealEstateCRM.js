...
export default function RealEstateCRM() {
  ...
  // 🔒 PAYWALL : abonnement requis pour le CRM Immobilier
  if (!hasCrmAccess(currentUser, userProfile, 'immo')) {
    return <CrmPaywall currentUser={currentUser} userProfile={userProfile} crmName="CRM Immobilier" crmType="immo" />;
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans print:hidden text-slate-800">
          ...
          <div className="flex items-center gap-3">
            {teamData?.logoUrl ? (
              <div className="relative">
                <img src={teamData.logoUrl} alt="Logo" className="h-11 w-11 object-contain rounded-2xl bg-white shadow-md border border-slate-200" />
          ...
          {isAdmin && (
            <>
              ...
              <button
                ...
                className="w-10 h-10 flex items-center justify-center bg-white border border-slate-200 hover:border-slate-300 text-slate-700 rounded-xl shadow-sm transition-all hover:shadow-md hover:bg-slate-50"
      ...
      {isTemplatesOpen && isAdmin && (
        <EmailTemplatesEditor
          adminUid={teamData?.adminUid || currentUser.uid}
          teamId={teamData?.id}
          teamCollection="teams_immo"
          teamName={teamData?.name}
          themeColor={themeColor}
